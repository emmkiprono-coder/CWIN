...
const seedBillingPeriods=[
  {id:"BP1",label:"Mar 1-15, 2026",start:"2026-03-01",end:"2026-03-15",status:"open"},
  {id:"BP2",label:"Feb 15-28, 2026",start:"2026-02-15",end:"2026-02-28",status:"closed"},
];
const seedInvoices=[
...
function BillingPage({invoices,setInvoices,clients,caregivers,rateCards,billingPeriods,setBillingPeriods,schedules,expenses}){
  ...
  return <div>
    ...
    <div className="card" style={{marginBottom:14}}>
      <div className="card-h"><h3>Billing Periods</h3></div>
      {billingPeriods.map(bp=> <div key={bp.id} style={{padding:"10px 20px",borderBottom:"var(--border-thin)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontWeight:600,fontSize:13}}>{bp.label}</div><div style={{fontSize:11,color:"var(--t2)"}}>{fmtD(bp.start)} — {fmtD(bp.end)}</div></div>
        <div style={{display:"flex",gap:6}}>
    ...
    <div className="card">
      ...
      <div className="tw"><table><thead><tr><th>Invoice #</th><th>Client</th><th>Period</th><th>Date</th><th>Due</th><th>Lines</th><th style={{textAlign:"right"}}>Subtotal</th><th style={{textAlign:"right"}}>Expenses</th><th style={{textAlign:"right"}}>Total</th><th>Status</th><th></th></tr></thead><tbody>
        {invoices.sort((a,b)=>b.date.localeCompare(a.date)).map(inv=>{const cl=clients.find(c=>c.id===inv.clientId);return <tr key={inv.id}>
          <td style={{fontWeight:700,fontFamily:"monospace"}}>{inv.id}</td>
          <td style={{fontWeight:600}}>{cl?.name}</td>
          <td style={{fontSize:11}}>{billingPeriods.find(b=>b.id===inv.periodId)?.label||"—"}</td>
          <td>{fmtD(inv.date)}</td><td>{fmtD(inv.dueDate)}</td>
          <td>{inv.lines.length}</td>
          <td style={{textAlign:"right"}}>{$(inv.subtotal)}</td>
          <td style={{textAlign:"right"}}>{inv.expenses>0?$(inv.expenses):"—"}</td>
    ...
    {sel&&(()=>{
      ...
      const handlePDF=()=>{
        ...
        const html="<html><head><title>Invoice "+sel.id+"</title><style>"+css+"</style></head><body>"+el.innerHTML+"</body></html>";
        // Use iframe approach (works in sandboxed environments)
        ...
        closeBtn.innerHTML="<button onclick='parent.document.getElementById(\"cwin-pdf-frame\").style.display=\"none\"' style='position:fixed;top:10px;right:10px;padding:8px 16px;background:#070707;color:#fff;border:none;cursor:pointer;font-size:13px;z-index:100000'>✕ Close</button><button onclick='window.print()' style='position:fixed;top:10px;right:90px;padding:8px 16px;background:#3c4f3d;color:#fff;border:none;cursor:pointer;font-size:13px;z-index:100000'>🖨 Print / Save PDF</button>";
